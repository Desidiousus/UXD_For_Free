...
.LNTHover:hover{background-color:black !important;color:#29A5FF !important;}
.hide{display:none;}
...
<script>
  ...
  function LDelViewer(DelID, GNTID) {
    $(".All").hide();
    $(".LocalDHeader").hide();
    $(".DelHeader").hide();
    $(".ReqHeader").hide();
  ...
  function LNTViewer(LNTID) {
    $(".All").hide();
    $(".LocalDHeader").hide();
    $(".LNTHover").hide();
    $(".DemHeader").hide();
    $(".ReqHeader").hide();
    $("#LNTHeader"+LNTID).show();
    $(".LNTSel"+LNTID).show();    

  }
  function LNTDelViewer(DelID, LNTID) {
    $(".All").hide();
    $(".LNTSel").hide();
    $(".LocalDHeader").hide();
    $(".LNTHover").hide();
...
$connGlobal = OpenCon();
$bab = "SELECT id, LocalNavTags FROM structure_deliverables 
where DeliverableGlobalNavTag = '".$_GET['activeWebPage']."' AND LocalNavTags IS NOT NULL";
$Result12 = $connGlobal->query($bab); 
while($row3 = $Result12->fetch_assoc()) {
  echo" <a  onclick='LNTViewer(".$row3['id'].")'class='btn btn-primary ' id='DelID11".$row3['id']."' href='#' style='padding-left:10px;width:100%;text-align:left;background-color:#29A5FF;color:black;' data-toggle='modal' data-target='#ORDModal'><h4 >***".$row3['id'].".) ".$row3['LocalNavTags']."***</h4></a>\r\n";
}
$sql82 = "SELECT DeliverableDeadline,DeliverableAssignedCollabIDs,DeliverableAssignedRequirementIDs,id,DeliverableTitle,
      ...
      DeliverableGlobalNavTag = '".$_GET['activeWebPage']."' AND DeliverableLocalNavTag IS NULL AND LocalNavTags IS NULL
      ORDER BY id ASC";
      $Result21 = $connGlobal->query($sql82); 
      while($row222 = $Result21->fetch_assoc()) {
        echo "<a  onclick='DelViewer(".$row222['id'].",".$row222['DeliverableGlobalNavTag'].")' class='btn btn-primary DelHov' href='#' style='padding-left:10px;background-color:#d98484;color:black;width:100%;text-align:left;' data-toggle='modal' id='DelID11".$row222['id']."' data-target='#ORDModal'><h4 >***".$row222['id'].".) ".$row222['DeliverableTitle']."***</h4></a>\r\n"
        ;
    }
echo '
<div id="ORDModal" class="modal fade" >
  <div class="modal-dialog "   style="margin:0;">
    ...
    $bab = "SELECT id, LocalNavTags FROM structure_deliverables where LocalNavTags IS NOT NULL  ";
    $Result12 = $connGlobal->query($bab); 
    while($row3 = $Result12->fetch_assoc()) {
      echo '
      <a  onclick="LNTViewer('.$row3['id'].')"class="LNTHover hide" id="LNTHeader'.$row3['id'].'" 
      href="#" style="padding-left:10px;padding-top:15px;background-color:#29A5FF;color:black;width:100%;text-align:left;">
        <h1 >'.$row3['id'].'.) '.$row3['LocalNavTags'].'</h1>
      </a>';
    }
    echo '
      </div>
    ...
    $sql82 = "SELECT DeliverableDeadline,DeliverableAssignedCollabIDs,DeliverableAssignedRequirementIDs,id,DeliverableTitle,
    DeliverableType,DeliverableDescription, DeliverableGlobalNavTag,DeliverableLocalNavTag FROM structure_deliverables WHERE 
    DeliverableLocalNavTag IS NOT NULL AND DeliverableGlobalNavTag = '".$_GET['activeWebPage']."' ORDER BY id ASC";
    $Result21 = $connGlobal->query($sql82); 
    while($row222 = $Result21->fetch_assoc()) {
      echo' <a  onclick="LNTDelViewer('.$row222['id'].','.$row222['DeliverableLocalNavTag'].')" 
      class="btn All btn-primary LNTSel LNTSel'.$row222['DeliverableLocalNavTag'].'" href="#" 
      style="background-color:#99DFFF;color:black;width:100%;text-align:left;">
...
echo '
  <div onclick="LDelViewer('.$row2['id'].','.$row2['DeliverableGlobalNavTag'].')" 
  class="LocalDHeader hide modal-header DelHov hide LDGNT'.$row2['DeliverableGlobalNavTag'].'" 
  href="#" style="padding-left:10px;padding-top:15px;background-color:#d98484;color:black;width:100%;text-align:left;
  padding-left:10px;" id ="DemHeader3'.$row2['id'].'">
